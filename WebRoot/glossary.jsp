...
<html>
	<head>
		...
		<link rel="stylesheet" type="text/css" href="public/style/glossary.css">
	...
	<body>
		...
		<div class="main-content">
			<div class="content">
				<div class="glossary" id="addGlossary">+</div>
			</div>
	...
	<script>
		...
		$.getAjax('GetAllGlossaryName',{},function(msg){
			msg = JSON.parse(msg);
			if(msg.type != 1){
				return;
			}
			var data = msg.oldGlossaryName.split(',');
			$('.content').setStyle('height',(parseInt((data.length+1)/5)+1)*380);
			for(var i=0;i<data.length;i++){
				$('.content').append($.create('<div class="glossary glo"></div>').setAttr('innerText',data[i]));
			}
			$('.glo').on('click',function(){
				window.location.assign('practice.jsp?glossaryname='+$(this).getAttr('innerText'));
			});
		})
	</script>
</html>